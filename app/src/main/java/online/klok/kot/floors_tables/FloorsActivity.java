...
import online.klok.kot.customAdapters.Table;
...
public class FloorsActivity extends AppCompatActivity implements FloorsAdapter.OnFloorItemClicked {
    ...
    private RecyclerView rvFloorsTables;
    ...
    private void initViews() {
        ...
        rvFloorsTables = (RecyclerView) findViewById(R.id.rv_floors_tables);
        rvFloorsTables.setLayoutManager(new GridLayoutManager(this, 3, GridLayoutManager.VERTICAL, false));
        rvFloorsTables.setAdapter(new FloorsAdapter(this, getFloorsItems()));

    }

    private ArrayList<FloorAndTablePOJO> getFloorsItems() {
        ArrayList<FloorAndTablePOJO> itemList = new ArrayList<>();

        for (int i = 0; i < floorModelList.size(); i++) {
            FloorAndTablePOJO floorAndTablePOJO = new FloorAndTablePOJO();
            floorAndTablePOJO.setName("" + floorModelList.get(i).getFloorName());
            floorAndTablePOJO.setId(floorModelList.get(i).getId());
            itemList.add(floorAndTablePOJO);
        }
...
//                StringBuffer finalBufferData = new StringBuffer();
        ...
        protected void onPostExecute(List<FloorModel> result) {
            ...
            rvFloorsTables.setAdapter(adapter);
        }
    }
}
